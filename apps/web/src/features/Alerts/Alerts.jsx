  ...
  return (
    <div className={styles.page}>
      <div className={styles.pageHeader}>
        ...
        <p className={styles.subtitle}>5-day IMD district warnings for {location.district}, {location.state}</p>
      </div>
      ...
      {dash?.alertColor >= 3 && (
        <div className={`${styles.alertBanner} ${styles[dash.alertSeverity]}`}>
          ...
          <div>
            ...
            <p>{dash.nowcastMsg || `Significant weather activity detected in ${location.district}.`}</p>
          </div>
        </div>
      )}
      ...
      <section className={styles.section}>
        ...
        <p className={styles.sectionSub}>Issued by IMD for {warn.District ?? location.district} · {warn.Date ?? new Date().toLocaleDateString('en-IN')}</p>
