...
export default function RiversReservoirs({ locationKey, apiData, loading }) {
  const location = LOCATIONS[locationKey] ?? LOCATIONS.hyderabad;
  const dash = deriveDashboardData(apiData, location);
  const aws = apiData?.aws?.data ?? [];

  return (
    <div className={styles.page}>
      <div className={styles.pageHeader}>
        <h1 className={styles.title}><Waves size={22} /> Rivers & Reservoirs</h1>
        <p className={styles.subtitle}>AWS station readings and rainfall-derived level estimates for {location.label}</p>
      </div>
      ...
      <div className={styles.riverCard}>
        <div className={`${styles.riverStatus} ${styles[dash?.riverSeverity ?? 'success']}`}>
          <Droplets size={32} />
          <div>
            <div className={styles.riverName}>{location.riverName}</div>
