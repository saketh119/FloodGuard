...
export default function CurrentFloods({ locationKey, apiData, loading, error, onRefresh }) {
    ...
    <div className={styles.page}>
      <div className={styles.pageHeader}>
        <div>
          ...
          <p className={styles.subtitle}>Live flood events derived from IMD nowcast and rainfall thresholds — {location.label}, {location.state}</p>
        </div>
      ...
      {loading && !dash ? (
        ...
        <div className={styles.error}>⚠️ Could not load data. Is the mock server running?</div>
      ) : filtered.length === 0 ? (
